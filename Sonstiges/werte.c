#pragma config(Sensor, S1,     HTCSL,          sensorAnalogActive)
#pragma config(Sensor, S2,     CenterL,        sensorAnalogActive)
#pragma config(Sensor, S3,     HTCSR,          sensorAnalogActive)
...
/* 		HTCS2readRGB(HTCSL, redL, greenL, blueL);
		...
		nxtDisplayCenteredTextLine(0, "L durch,c:%d",SensorValue[HTCSL]);
		nxtDisplayCenteredTextLine(1, "center Light:%d",SensorValue[CenterL]);
		nxtDisplayCenteredTextLine(2, "R durch,c:%d",SensorValue[HTCSR]);
